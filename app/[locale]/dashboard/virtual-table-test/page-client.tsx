...
export function VirtualTableDemoClient() {
  ...
  const columns = React.useMemo<ColumnDef<MockData>[]>(
    () => [
      {
        accessorKey: "id",
        header: ({ column }) => (
          <VirtualDataTableColumnHeader column={column} title={gt("ID")} />
        ),
        cell: ({ row }) => (
      ...
      },
      {
        accessorKey: "status",
        header: ({ column }) => (
          <VirtualDataTableColumnHeader column={column} title={gt("Status")} />
        ),
        meta: {
          virtualTable: {
            width: "10rem",
      ...
      },
      {
        accessorKey: "value",
        header: ({ column }) => (
          <VirtualDataTableColumnHeader column={column} title={gt("Value")} />
        ),
        cell: ({ row }) => {
  ...
  return (
    <VirtualDataTable
      ...
      data={data}
      description={gt(
        "Uses window scrolling against 100,000 rows so you can verify the virtualizer without an inner scroll container."
      )}
      scrollMode="window"

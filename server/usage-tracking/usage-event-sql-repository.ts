...
export class UsageEventSqlRepository {
	...
	buildFetchSurroundingFlatRowsQuery(input: SurroundingUsageEventsInput) {
      ...
      with ranked_events as (
        select
          ...
          ${apiKeys.lastFour} as "apiKeyLastFour",
          ${apiKeys.name} as "apiKeyName",
          ${apiKeys.keyPrefix} as "apiKeyPrefix",
          ${trackableApiUsageEvents.id} as id,
          ${trackableApiUsageEvents.metadata} as metadata,
          ${trackableApiUsageEvents.occurredAt} as "occurredAt",
      ...
      select
        ranked_events."apiKeyId",
        ranked_events."apiKeyLastFour",
        ranked_events."apiKeyName",
        ranked_events."apiKeyPrefix",
        ranked_events.id,
        ranked_events.metadata,
        ranked_events."occurredAt",
        ranked_events.payload
      from ranked_events
      inner join anchor_event on true
      where ranked_events."rowIndex" between greatest(anchor_event."rowIndex" - ${input.beforeCount}, 1)
        and anchor_event."rowIndex" + ${input.afterCount}
      order by ranked_events."rowIndex" asc
    `;
	}
		...
		switch (field.kind) {
			...
			case "metadata":
				return field.path.length === 0
					...
					: this.buildJsonValueSql(
							trackableApiUsageEvents.metadata,
							field.path,
						);
			default:
				return null;
	...
	private buildFlatRowSelection(eventSortValueSql: SQL<string | null>) {
		return {
			apiKeyId: sql<string>`${apiKeys.id}`.as("apiKeyId"),
			apiKeyLastFour: sql<string>`${apiKeys.lastFour}`.as("apiKeyLastFour"),
			apiKeyName: sql<string>`${apiKeys.name}`.as("apiKeyName"),
			apiKeyPrefix: sql<string>`${apiKeys.keyPrefix}`.as("apiKeyPrefix"),
			id: sql<string>`${trackableApiUsageEvents.id}`.as("id"),
			metadata: sql`${trackableApiUsageEvents.metadata}`.as("metadata"),
			occurredAt: sql<Date>`${trackableApiUsageEvents.occurredAt}`.as("occurredAt"),
			payload: sql<
				Record<string, unknown>
			>`${trackableApiUsageEvents.payload}`.as("payload"),
			sortValue: eventSortValueSql.as("sortValue"),
		};
	}
	...
	private buildGroupedSourceSubquery(
		...
		return this.queryBuilder
			.select({
				...
				groupIdentity: groupIdentitySql.as("groupIdentity"),
				groupValue: groupValueSql.as("groupValue"),
	...
	private buildGroupedCursorFilter(
		...
		groupedSubquery: ReturnType<
			UsageEventSqlRepository["buildGroupedSubquery"]
		>,
	) {
		if (!cursor) {
			return undefined;
